      ...
      <div className="max-w-6xl mx-auto px-4 py-6">
        ...
        {showWelcome && (
          <div className={`rounded-xl border p-4 mb-6 relative ${darkMode ? 'bg-amber-950/30 border-amber-900/50' : 'bg-amber-50 border-amber-200'}`}>
            ...
            <p className={`text-sm font-IBM Plex sans JP ${darkMode ? 'text-violet-500' : 'text-violet-700'}`}>
              主に近代西洋の詩と散文を収録。フランス語・ドイツ語・イタリア語・ロシア語・英語の原文と日本語仮訳を並べて比較し、自分の訳文も記録できます。
            </p>
            <p className={`text-sm font-IBM Plex sans JP ${darkMode ? 'text-red-500' : 'text-red-700'}`}>
              掲載の日本語訳は学習補助のための試訳であり、確定した翻訳ではありません。
            </p>
            <p className={`text-m font-IBM Plex sans JP ${darkMode ? 'text-violet-300' : 'text-violet-900'}`}>
